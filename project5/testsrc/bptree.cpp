...
TEST_SUITE(BPTreeTest::assignment, {
    // TOOD: impl test
})

TEST_SUITE(BPTreeTest::reset_file, {
    // TODO: impl test
})
...
TEST_SUITE(BPTreeTest::find_leaf, {
    ...
    for (int i = 0; i < 40; ++i) {
        if (i && i % 2 == 0) {
            pagenum = buf.read([&](Page const& page) {
...
TEST_SUITE(BPTreeTest::insert_to_leaf, {
    ...
    Ubuffer buf = bpt.create_page(true);
    pagenum_t bufnum = buf.to_pagenum();
...
TEST_SUITE(BPTreeTest::insert_and_split_leaf, {
    ...
    pagenum_t leafnum = leaf.to_pagenum();

    Record record;

    // case 0. last one
    constexpr int size = 4;
    TEST_SUCCESS(leaf.check_and_reload());
...
TEST_SUITE(BPTreeTest::insert_and_split_node, {
    ...
    for (int i = -1; i < internal_order - 1; ++i) {
        ...
        pages[i + 1] = tmp.to_pagenum();
        tmp.write_void([&](Page& page) {
        ...
        if (i != -1) {
            leaf.write_void([&](Page& page) {
                page.page_header().special_page_number = tmp.to_pagenum();
            });
        }
        leaf = std::move(tmp);
        ...
        node.write_void([&](Page& page) {
            if (i == -1) {
                page.page_header().special_page_number = leaf.to_pagenum();
            } else {
                Internal& ent = page.entries()[i];
                ent.key = i * leaf_order;
                ent.pagenum = leaf.to_pagenum();
            }
        });
    }

    TEST_SUCCESS(node.check_and_reload());
    for (int i = -1; i < internal_order - 1; ++i) {
        if (i == -1) {
    ...
    pages[internal_order] = leaf.to_pagenum();
    leaf.write_void([&](Page& page) {
        page.page_header().parent_page_number = nodenum;
        page.page_header().number_of_keys = leaf_order - 1;
    });
    ...
    for (int i = -1; i < split - 1; ++i) {
        ...
        TEST_SUCCESS(bpt.buffering(pages[i + 1]).read([&](Page const& page) {
    ...
    for (int i = -1; i < expected; ++i) {
        ...
        TEST_SUCCESS(
            bpt.buffering(pages[split + 1 + i]).read([&](Page const& page) {
...
TEST_SUITE(BPTreeTest::insert_to_parent, {
    ...
    parent.write_void([&](Page& page) {
        ...
        page.page_header().special_page_number = temporal.to_pagenum();
        page.entries()[0].key = 10;
        page.entries()[0].pagenum = leftnum;
    });
    temporal.write_void([&](Page& page) {
    ...
    TEST(parent.page().page_header().special_page_number == temporal.to_pagenum());
...
TEST_SUITE(BPTreeTest::insert_new_root, {
    ...
    TEST(buf.page().page_header().special_page_number == left.to_pagenum());
    TEST(buf.page().entries()[0].key == 10);
    TEST(buf.page().entries()[0].pagenum == right.to_pagenum());

    TEST_SUCCESS(left.check_and_reload());
    TEST(left.page().page_header().parent_page_number == buf.to_pagenum());
    TEST_SUCCESS(right.check_and_reload());
    TEST(right.page().page_header().parent_page_number == buf.to_pagenum());
...
int bptree_test() {
    ...
    return BPTreeTest::constructor_test()
        ...
        && BPTreeTest::assignment_test()
        && BPTreeTest::reset_file_test()
